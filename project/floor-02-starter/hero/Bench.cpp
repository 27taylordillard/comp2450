...
//
// Note the loop below rather than two plain recursive calls. Naively,
// this function recurses once per element on sorted input — N frames
// deep — which overflows the stack and CRASHES long before the timing
// gets interesting. So we recurse into the smaller side and iterate on
// the larger one. That caps stack depth at O(log n) while leaving the
// COMPARISON count untouched: the pathology is fully preserved, only the
// crash is gone. This is the same trick a production quicksort uses.
void badQuicksortImpl(std::vector<Item>& v,
    ...
    while (lo < hi) {
        ...
        // Left side is [lo, p-1] (size p - lo); right is [p+1, hi] (size
        // hi - p). Recurse into whichever is smaller, then loop on the
        // other. `p - 1` with std::size_t underflows when p == lo, so
        // both branches guard it — the same trap your own quicksort has
        // to navigate.
        if (p - lo < hi - p) {
...
void runSortBenchmarkSweep(SortBenchOptions opts) {
    ...
    // you nothing that N=10000 hasn't already. Cap it, and say so, rather
