...
//
// This is the post-Floor-9½ reference codebase plus this week's Floor-10
...
// `potato`, the general Tree<T> with `taxonomy`/`lineage`, the full Floor 9
// BST<T> (insert, find, in-order, Rule of Three, remove), AND the Floor 9½
// balance work (balanceFactor, isBalanced, the rotations, balanceRoot).
// `catalogue`, `recall`, `forget`, `balance`, `benchmark bst`, and
// `benchmark balance` all work.
//
// This week you STOP building structures and start USING them. For five floors
// you hand-rolled pointer structures; Floor 9½ taught you not to hand-roll the
// balanced tree but to reach for std::set / std::map, which ARE balanced trees.
// Floor 10 is where you reach: you write CLIENT code against std::set,
// std::map, and std::pair — you do NOT implement an associative container. The
// payoff, out loud: the library already gives you the balanced tree you just
// studied, and a key->value index on top of it, for free.
//
// What is NEW for Floor 10 (all in bestiary/Registry.h — six small bodies):
//   * index / knows         — build the indexes once; ask membership in one
//                             call. Stubbed (Mon).
//   * recall / hasWeakness / countWeakness — the indexed lookup, set membership,
//                             and count-without-the-Phantom-Key. Stubbed (Wed).
//   * forget                — remove a name, keeping three containers
//                             consistent. Stubbed (Fri).
//   * bestiary/RegistryTests.* — `selftest registry` exercises all of it (6 phases).
//   * `knows` / `lookup` / `strike` / `names` / `weaknesses` / `weakness`
//                             — the REPL demos. (Floor 9's `recall`/`forget`
//                             commands stay bound to the BST, untouched —
//                             `lookup`/`strike` are the Registry's verbs.)
//   * `benchmark names [N]` — std::map::find vs your BST::find vs Floor 1's
//                             linear scan over the same data. The payoff.
//
// What you do this week (see the Floor 10 page):
//   Mon — index, knows; `selftest registry` phases 1–2 pass; `knows <name>`
//         answers, and `weaknesses` shows the distinct set is already sorted.
//   Wed — recall, hasWeakness, countWeakness; phases 3–5 pass; `names` lists
//         the bestiary in order via structured bindings (the std::pair beat).
//   Fri — forget; phase 6 passes; `benchmark names` lands the lesson.
...
int main() {
    ...
    std::cout << "\nWelcome, " << hero.heroName << ".\n"
              << "Scrivener McCown stands, for once, at rest — no chart to straighten.\n"
              << "Behind him a wall of name-plates, each with its meaning engraved\n"
              << "beside it, and a smaller shelf listing every KIND of weakness once.\n"
              << "\"On the last two floors I taught you to build the shelf and to keep\n"
              << "it level,\" he says. \"I will not make you build this one. It is\n"
              << "already built — better than I could, and it never leans. I only have\n"
              << "to know the name.\" He speaks a name to the shelf, and the plate answers.\n\n";
        ...
        try {
            ...
            // `recall`/`forget` stay bound to the BST and are untouched.
                ...
                if (rest == "chain") {
                    ...
                    std::cout << "Usage: selftest chain | iterator | stack | queue | tree | bst | balance | registry\n";
                }
            }
