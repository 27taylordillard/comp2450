...
//
// This is the post-Floor-11 reference baseline. Everything Floors 0–11
// delivered ships COMPLETE and WORKING here: Bag<T>, the sorts, the boss
// battle, Chain<T> with iterators, Stack<T> and Queue<T>, isBalanced +
// `lint`, `undo`, `provoke`/`simulate`, `potato`, the general Tree<T> with
// `taxonomy`/`lineage`, the full Floor 9 BST<T> + Floor 9½ balancing, the
// Floor 10 Registry (std::set/std::map), AND the Floor 11 HashTable — your
// Hall of Whispers work, filled here (no TODOs left in hero/HashTable.h).
// `catalogue`, `recall`, `forget`, `balance`, `knows`, `lookup`, `strike`,
// `names`, `weaknesses`, `weakness`, `hash`, `buckets`, `whisper`, `banish`,
// `load`, and every `benchmark` all work.
//
// The ONLY stub in this drop is runTreeWarden in battle/Battle.cpp — that is
// Trial III, the code you write for this midterm. The `battle trees` command
// below is already wired to call it; `battle warden` still plays Midterm 1's
// reference encounter, so both battles are here.
//
// This is a WARDEN (a take-home midterm boss), not a floor: no Mon/Wed/Fri
// split, no new reading. The four halls above — the classification Tree
// (Floor 8), the searchable-but-leaning BST (Floor 9/9½), the name-shelf
// Registry (Floor 10), and the whisper-wall HashTable (Floor 11) — are the
// four "keepers" of this gate, and each must show up as a load-bearing call
// in your runTreeWarden. See the four-tie checklist at the top of that stub.
//
// Before you touch anything, read README.md in this folder: it walks you
// through the Path A / Path B choice — extend your own Floor 11 project
// (preferred), or work from this starter if yours won't build. Build & run
// instructions are there too (cmake -B build, etc.).
...
int main() {
    ...
    // case). `catalogue`/`recall`/`forget`/`balance` already work — and the
    // Warden's Recall rides this tree's find()/height()/balance judgement.
    ...
    // and the Warden's Ward rides this Registry's weakness set + count.
    ...
    // already work — and the Warden's free Whisper rides this table's find().
    HashTable whispers(bestiary);

    // Floor 8 (finished reference): McCown's classification tree, built from
    // data/taxonomy.txt. `taxonomy`/`taxonomy --dfs/--bfs`/`lineage` already
    // work — and the Warden's menagerie marshals in this tree's levelOrder(),
    // while a correct lineage() exposes the Warden itself.
        ...
        try {
            ...
            // the HashTable's verbs; nothing earlier uses them.
                ...
                if (which == "warden") {
                    ...
                    // Midterm 3 — the Warden of the Trees. The four indexes are
                    // passed straight through by const& (the battle never
                    // mutates them; the banish works on a LOCAL copy of the hash
                    // table). The menagerie length is a fixed constant inside
                    // runTreeWarden — `battle trees` takes no argument.
