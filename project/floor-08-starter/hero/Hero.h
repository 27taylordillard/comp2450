...
// hero/Hero.h — UNCHANGED for Floor 8. The Hero already owns one of every
// linear shape (Bag, Chain, Stack, Queue). Floor 8's new structure is a
// TREE, but McCown's classification tree belongs to the catalogue, not the
// hero — it lives as a local Tree<std::string> in main.cpp, the same way
// the `bestiary` does. So this file is exactly as it left Floor 7.
//
// (Floor 7 recap: the Queue<std::string> enemyActionQueue feeds the
// `provoke`/`simulate` commands; the Stack<UndoAction> feeds `undo`.)
//
