...
// ---- Floor 11: your HashTable vs unordered_map vs std::map vs BST vs linear --
// Builds the SAME synthetic bestiary as the student's HashTable (from a Bag), a
// std::unordered_map, a std::map, a shuffled BST, and a sorted vector, then races
// the worst-case (last) and absent lookups across all of them. The HashTable
// column prints `--` if HashTable::insert is still stubbed (size()==0), so the
// command works from day one; every other column always runs. Two rows per call.
void runWhispersBenchmark(std::size_t n, std::size_t iterations = 1000);

// Floor 11 sweep: N = 100, 1'000, 10'000, 100'000.
void runWhispersBenchmarkSweep();

// ---- Floor 12 (finale): adjacency LIST vs adjacency MATRIX traversal -------
// Builds the SAME synthetic SPARSE graph of V rooms as an adjacency matrix
// (built INLINE), a reference adjacency list (built INLINE), and the student's
// Graph, then times a full breadth-first crossing of each from a start vertex
// that varies per iteration. The YourGraph column prints `--` if Graph::bfsOrder
// is still stubbed (returns empty), so the command works from day one; the
// matrix and list columns always run. One row per call.
void runCrossingBenchmark(std::size_t n, std::size_t iterations = 100);

// Floor 12 sweep: V = 50, 200, 800, 3'200 (kept modest — the matrix is O(V²)
// space, so very large V would allocate a V×V grid).
