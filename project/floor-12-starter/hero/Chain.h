...
//
// Floor 11 addition (still reference code — you do NOT edit it): a templated
// eraseFirst(pred) near the bottom, the "remove the first matching node"
// helper the Hall of Whispers' HashTable::erase calls on a bucket's chain.
...
public:
    ...
    // Floor 11 reference helper (you do NOT edit this file). Remove the FIRST
    // node whose data satisfies pred, fixing up the prev/next links and the
    // head_/tail_/size_ bookkeeping. Returns true iff a node was removed. This
    // is the general "erase by predicate" that pop_front/pop_back don't give
    // you; the hash table's erase() calls it with a name-matching predicate, so
    // removing one item from a bucket's chain is a single line and any colliding
    // neighbors stay put.
    template <typename Pred>
    bool eraseFirst(Pred pred) {
        for (Node* p = head_; p != nullptr; p = p->next) {
            if (!pred(p->data)) continue;
