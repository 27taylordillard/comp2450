...
//
// This is the post-Floor-10 reference codebase plus this week's Floor-11 stubs.
// Everything Floors 0–10 delivered is in place and WORKING (no longer stubbed):
...
// 9½ balancing, AND the Floor 10 Registry (std::set/std::map). `catalogue`,
// `recall`, `forget`, `balance`, `knows`, `lookup`, `strike`, `names`,
// `weaknesses`, `weakness`, and every `benchmark` so far all work.
//
// This week you build the LAST data structure you will build by hand — and it
// is made out of one you already built. Floor 10 walked a balanced tree in
// O(log n) by keeping everything in ORDER. The Hall of Whispers answers in O(1)
// AVERAGE by giving up order entirely: it COMPUTES a bucket from the name (a
// hash) and goes straight there. A whisper, not a walk. The bucket is YOUR
// Chain<Monster> from Floor 4, so a hash table is literally an array of the
// linked list you wrote five floors ago. Then, exactly as Floor 9's BST turned
// out to be std::map, you meet std::unordered_map and see you just built it.
//
// What is NEW for Floor 11 (all in hero/HashTable.h — six small bodies):
//   * hashKey / bucketIndex — compute a hash CODE for a name (multiplicative
//                             string hash), then reduce it to a bucket. Stubbed (Mon).
//   * insert                — put a monster in its home bucket, no duplicates,
//                             grow when crowded. Stubbed (Mon→Wed→Fri).
//   * find / erase          — scan ONE bucket's chain to look up / remove. Stubbed (Wed).
//   * rehashIfNeeded        — double the buckets when the load factor climbs;
//                             plus the one re-file line in the rehash driver. Stubbed (Fri).
//   * hero/HashTableTests.* — `selftest whispers` exercises all of it (6 phases).
//   * hero/Chain.h          — gains a reference-only eraseFirst(pred) helper your
//                             erase() calls. (You still do not edit Chain.h.)
//   * `hash` / `buckets` / `whisper` / `banish` / `load`
//                             — the REPL demos (fresh verbs; nothing earlier uses them).
//   * `benchmark whispers [N]` — your HashTable::find vs std::unordered_map vs
//                             std::map vs BST vs linear. The payoff.
//
// What you do this week (see the Floor 11 page):
//   Mon — hashKey, bucketIndex, basic insert; `selftest whispers` phase 1
//         passes; `hash <name>` shows the computed bucket; `buckets` shows the 15
//         monsters scattered across slots, with at least one COLLISION.
//   Wed — finish insert (no dups), find, erase; phases 2–5 pass; `whisper
//         <name>` finds by scanning one bucket, `banish <name>` removes one item.
//   Fri — rehashIfNeeded; phase 6 passes; `load` shows the load factor; force
//         growth and watch the buckets double; `benchmark whispers` lands it.
...
int main() {
    ...
    std::cout << "\nWelcome, " << hero.heroName << ".\n"
              << "The orderly shelves of the Hall of Names are behind you. This hall has\n"
              << "no alphabet at all — only a wall of NICHES, and names sit wherever a\n"
              << "whispered word sends them. Scrivener McCown lowers his voice. \"Up there\n"
              << "I kept everything in its place so I could find it. Here I keep nothing\n"
              << "in its place — and I find it faster. I do not look for the name. I\n"
              << "SPEAK it, and the niche it belongs to answers. The trick is the\n"
              << "speaking: the same word must always send to the same niche.\" He pauses.\n"
              << "\"The danger is when two names whisper to the same niche — a collision.\n"
              << "What saves you is that each niche keeps a little chain of all that\n"
              << "landed there. You built that chain five floors ago.\"\n\n";
    ...
    // Floor 10: the bestiary as standard associative containers — a
    // std::map keyed by name and a std::set of distinct weaknesses. The
    // balanced tree of Floor 9½, handed to you finished by the library.
    // Added IN PARALLEL: the Bag and the BST above are untouched. Until your
    // Registry::index works (Monday), this holds nothing and `names` is empty.
    Registry registry(bestiary);

    // Floor 11: the bestiary as a hand-built hash table — an array of the
    // Chain<Monster> buckets you wrote on Floor 4, with your own hash function.
    // The Hall of Names walked a balanced tree (O(log n), ordered); the Hall of
    // Whispers COMPUTES the bucket and goes straight there (O(1) avg, unordered).
    // Added IN PARALLEL: the Bag, the BST, and the Registry above are untouched.
    // It starts with 8 buckets; as your insert() files all 15 monsters, the load
    // factor crosses the threshold and the table GROWS itself (rehash) — try
    // `load` and `buckets` to watch it. Until your insert() works (Monday), this
    // holds nothing and `buckets` is empty.
        ...
        try {
            ...
            // the HashTable's verbs; nothing earlier uses them.
                ...
                if (rest == "chain") {
                    ...
                    std::cout << "Usage: selftest chain | iterator | stack | queue | tree | bst | balance | registry | whispers\n";
                }
            }
