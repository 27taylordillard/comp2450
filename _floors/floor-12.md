...
<figure class="diagram">
  ...
  <figcaption>The <strong>same map, the same start, two orders</strong>. <strong>BFS</strong> (left) rides your Floor 7 <code>Queue</code> — FIFO — and fans out in rings: the start, then everything one door away, then two doors away (§13.7). <strong>DFS</strong> (right) rides your Floor 6 <code>Stack</code> — LIFO — and plunges down one passage to its end before backtracking (§13.8). The data structure is the <em>only</em> difference; the <strong>visited set</strong> (the hash set from Floor 11) is identical for both, and it is what lets either one cross a map that has a cycle. <em>FIFO fans out; LIFO plunges down — exactly the contrast you learned on Floors 6 and 7.</em></figcaption>
</figure>

There is a fourth thing, and it is the trap that gives this floor its monster — the most dangerous bug in the course, because nothing crashes. A graph is not a tree: it has no leaves to stop you, no single root, and its passages can loop. Follow them naively and a single **cycle** (§13.5) — `Entrance–Goblin–Skeleton–Entrance` — feeds you back to where you started, and you walk it forever. The program does not error out; it simply **never returns**, discovered when the demo hangs. The cure is a **visited set**: a `std::unordered_set` (your Floor 11 hash set) that remembers every room the first time you discover it, so you never enqueue or push it twice. Mark a room visited *before* it enters the frontier, check membership *before* you add — and the same set serves both BFS and DFS, because the container differs but the *memory* is identical. We meet that one head-on all week; it is the last boss.
...
- **Keep a visited set.** Mark every room the first time you discover it — a `std::unordered_set` (your Floor 11 hash set) keyed on the room name — and never enqueue or push it again. §13.7 names this: BFS proceeds "without revisiting a vertex." Without the set, a single cycle is an infinite loop.
- **Check membership *before* you add to the frontier, not after.** Mark a room visited as you *enqueue* or *push* it, so it can never enter the queue or stack twice. Recording it only when you dequeue or pop — without the enqueue-time guard — lets a room be added many times before its first visit (correct by luck on a tree, catastrophic on any graph with a cycle).
