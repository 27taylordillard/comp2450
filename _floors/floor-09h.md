...
- Before **Monday** — §10.1 *AVL: A balanced tree*. The one definition that matters: a node's **balance factor** is the height of its left subtree minus the height of its right, and an AVL tree keeps every balance factor in {−1, 0, +1}. Notice the book's height convention treats an empty subtree as −1 — that is the convention your `balanceFactor` must use, and the reason a three-node stick is *unbalanced*, not balanced.
